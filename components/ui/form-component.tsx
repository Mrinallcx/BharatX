...
const ModelSwitcher: React.FC<ModelSwitcherProps> = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      // All models available to everyone - no Pro checks needed
                    ...
                    <div className="flex flex-col gap-2 min-w-0 flex-1">
                      {selectedProModel?.label ? (
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <span className="text-xl sm:text-2xl font-be-vietnam-pro">BharatX</span>
                          <ProBadge className="!text-white !bg-white/20 !ring-white/30 font-extralight" />
                        </div>
                      )}
                    </div>
...
const GroupModeToggle: React.FC<GroupSelectorProps> = React.memo(
  ({ selectedGroup, onGroupSelect, status, onOpenSettings, isProUser }) => {
    ...
    const visibleGroups = useMemo(
      () =>
        dynamicSearchGroups.filter((group) => {
          ...
          // All groups available to everyone - no authentication required
    ...
    const handleOpenChange = useCallback(
      (newOpen: boolean) => {
        // Extreme mode is now available to everyone - no restrictions
        setOpen(newOpen);
      },
      [setOpen],
    );
            ...
            {orderedVisibleGroups.map((group) => {
              ...
              >
                <div className="flex items-center gap-2 min-w-0 flex-1 pr-4">
                  <GroupIcon groupId={group.id} icon={group.icon} size={30} />
              ...
              <Tooltip open={tooltipOpen} onOpenChange={setTooltipOpen}>
                <TooltipTrigger asChild>
                  <DrawerTrigger asChild>
                    ...
                    >
                      {mounted && selectedGroupData && !isExtreme && (
                        <>
                          <GroupIcon groupId={selectedGroupData.id} icon={selectedGroupData.icon} size={30} />
                    ...
                    {orderedVisibleGroups.map((group) => {
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <GroupIcon groupId={group.id} icon={group.icon} size={24} />
                          <div className="flex flex-col min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <p className="font-semibold text-xs">
                      {isExtreme ? 'Extreme Search Active' : 'Extreme Search'}
                    </p>
                  </div>
                  <p className="text-[11px] leading-snug text-secondary">
      ...
      if (!isEnhancing && !isTypewriting) {
        ...
        if (group.id === 'extreme' && !supportsExtremeMode(selectedModel)) {
          ...
          const extremeModels = allModels.filter((model) => {
            ...
            // All models available to everyone - no pro requirement
            ...
            >
              ...
              >
                ...
                <div className={cn('flex items-center flex-shrink-0 gap-1.5')}>
                  ...
                  ) : false && input.length === 0 && attachments.length === 0 && !isEnhancing && !isTypewriting ? (
