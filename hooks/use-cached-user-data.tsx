...
export function useCachedUserData() {
  ...
  const [cachedUser, setCachedUser] = useLocalStorage<ComprehensiveUserData | null>('bharatx-user-data', null);
  ...
  // Recalculate derived properties - all users have Pro access
  const isProUser = true; // All users have Pro access - all features free
  const proSource = user?.proSource || 'none';
  const subscriptionStatus = 'active'; // All users treated as having active subscription
