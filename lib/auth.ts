...
// Initialize DodoPayments client only if API key is provided
...
export const auth = betterAuth({
  ...
  plugins: [
    // Only add DodoPayments plugin if API key is provided
    ...(dodoPayments && process.env.DODO_PAYMENTS_API_KEY
      ? [
          dodopayments({
            client: dodoPayments,
    ...
    : []),
    // Only add Polar plugin if access token is provided
    ...(polarClient && process.env.POLAR_ACCESS_TOKEN
      ? [
          polar({
            client: polarClient,
      createCustomerOnSignUp: true,
